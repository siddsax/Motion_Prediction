...
for iteration =   iter_use
    ...
    for N = 0:7
        ...
        if exist([dirname,'/ground_truth_unnorm_', num2str(N) ],'file') ~= 2
            disp([dirname,'/ground_truth_unnorm_', num2str(N)])
            continue
        end;
        
        f=csvread([dirname,'/ground_truth_unnorm_', num2str(N)]);
        fstd = std(f,1);
        ...
        if exist([dirname,'/predicted_test_full_unnorm_',num2str(N)],'file') ~= 2
            continue
        end;       
        f=csvread([dirname,'/predicted_test_full_unnorm_',num2str(N)]);
        disp([dirname,'/predicted_test_full_unnorm_', num2str(N)])
        expchannels = revertCoordinateSpace(f,R0,T0);
