...
def GCNNmodelRegression(preGraphNets,nodeList,nodeFeatureLength, temporalNodeFeatureLength,new_idx,featureRange,time_steps,Num_samples):
	...
	#----------------------------------------------------------------------------------------
	gcnn = GCNN(graphLayers,finalLayer,preGraphNets,nodeNames,temporalNodeRNN,nodeRNNs,topLayer,euclidean_loss,nodeLabels,learning_rate,adjacency,new_idx,featureRange,time_steps,Num_samples,clipnorm=args.clipnorm,update_type=gradient_method,weight_decay=args.weight_decay)
	return gcnn
...
def trainGCNN():
	...
	time_steps,Num_samples,D = np.shape(trX['right_arm'])
	...
	if args.use_pretrained == 1:
		...
		args.iter_to_load = 0
		gcnn = GCNNmodelRegression(preGraphNets,nodeList,nodeFeatureLength,temporalNodeFeatureLength,new_idx,featureRange,time_steps,Num_samples)
